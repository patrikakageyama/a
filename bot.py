...
def _check_and_close_ad(page):
    try:
        ...
        # 2. Check main page for 'Close' button
...
def solve_captcha(page):
    ...
    while True:
        ...
        # Look specifically for the captcha image by grabbing the first image inside an element (avoids grabbing ad thumbs-up)
        captcha_img = page.locator("div.col-sm-3 img, img").first
        ...
        try:
            ...
            if len(tex_ocr) < 3:
                ...
                try:
                    # Look for the refresh button (usually next to captcha)
                    page.locator("a[href='']").first.click(timeout=1000)
                except:
                    pass
                time.sleep(3)
                continue
...
def run(playwright):
    ...
    try:
        # Use the specific class for the Views button
        btn_locator = page.locator("button.t-views-button")
        btn_locator.click(timeout=10000)
        print("[*] Clicked Views! Waiting 5 seconds before proceeding...")
        time.sleep(5)
        _check_and_close_ad(page)
    except Exception as e:
    ...
    while True:
        ...
        if "views are sending" in active_panel.lower():
            print("[*] Views are currently sending... waiting.")
            check_count = 0
            continue
